...
int main(void)
{
    ...
    printf("\n\nWelcome to Hagos's sort.c, compiled on %s %s.\n", __DATE__, __TIME__);
...
ListItem *SelectionSort(ListItem* list)
{
    ListItem *ouritem = LinkedListGetFirst(list);
    while (ouritem != LinkedListGetLast(list))
    {
        ListItem *sort=ouritem->nextItem;
        while (sort!=NULL){
            if(strcmp(ouritem->data,sort->data)>0){
                LinkedListSwapData(ouritem, sort);
            }sort=sort->nextItem;
        }ouritem=ouritem->nextItem;
    }
    ListItem *sortsecond=LinkedListGetFirst(ouritem);
    return sortsecond;
}
...
ListItem *InsertionSort(ListItem* list)
{
    ...
    while (list->previousItem != NULL){
        printf("list begins");
        ListItem *sorted=list->previousItem;
        if (strcmp(sorted->data, list->data) < 0){
            LinkedListSwapData(list, sorted);
        }
        else{
            ListItem *finalscan=list;
            while(finalscan!=NULL){
                if (strcmp(finalscan->nextItem->data, sorted->data)>0){
                    break;
                }else{
                    finalscan=finalscan->nextItem;
                }
            }
            LinkedListRemove(sorted);
            LinkedListCreateAfter(finalscan, sorted->data);
            printf("The Loop");
        }
    }
    printf("List Returned");
    return list;
}

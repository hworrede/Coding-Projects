...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    while True:
        ...
        # # Extracting the request method, path and HTTP version
        # words = lines[0].split(' ')
        # checking_request = words[0]
        # checking_requestPath = words[1]
        # checking_version = words[2]
        
        # # Validating the HTTP request method
        # if checking_request != 'GET':
        #     conn.sendall(b'HTTP/1.1 501 Not Implemented\r\n\r\n')
        #     conn.close()
        
        # # Validating the HTTP version
        # if checking_version != 'HTTP/1.1':
     ...
     # Handling the request
            try:
                ...
                txtfile = open("hworredeSocketOutput.txt", "a", newline="\n")
                txtfile.write(response_body)
                txtfile.close()
                csvfile = open("hworredeSocketOutput.csv", "a", newline="\n")
                writer = csv.writer(csvfile)
                writer.writerow(["Client request served", "4-tuple:", HOST, args.Port_number, client_ip, client_port, "Requested URL", args.Redirectory, status, "Bytes transmitted:", content_len])
                csvfile.close()
# writer.writerow(["Client request served", "4-tuple:", host_ip, host_port, client_ip, client_port, "Requested URL", uri, status, "Bytes transmitted:", contentLen])
